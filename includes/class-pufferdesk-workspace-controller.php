...
final class PufferDesk_Workspace_Controller {
	...
	public function save_state() {
		...
		if ( is_wp_error( $state ) ) {
			...
			wp_send_json_error(
				array(
					'conflict'            => 'pufferdesk_workspace_conflict' === $state->get_error_code(),
	...
	private function read_json_payload( $key ) {
		...
		// phpcs:ignore WordPress.Security.NonceVerification.Missing -- AJAX handlers verify the settings nonce before calling this helper. The decoded payload is sanitized by PufferDesk_Workspace_State.
