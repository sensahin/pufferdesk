...
final class PufferDesk_Shell_Context {
	...
	public function get() {
		...
		$system_folders       = $this->virtual_filesystem->get_system_folders( $theme );
		$app_folders          = $this->app_registry->get_folders( $apps );
		$folders              = array_merge( $system_folders, $app_folders );
		$desktop_icon_folders = array_merge(
			$this->virtual_filesystem->get_desktop_system_folders( $theme ),
			$app_folders
		);
