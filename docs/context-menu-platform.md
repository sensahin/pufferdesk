...
| Sticky note | Widget/document object | Copy, Paste on note chrome/root; Cut appears only outside PufferDesk/Finder-style menus; native browser menu in editable content | `assets/js/core/desktop/sticky-notes.js`, `assets/js/core/shell/context-menu.js` | Clipboard commands and document service | Sticky note controls document editing UI; shell menu uses shared skin outside editor content | Editable content uses `data-pdk-native-context-menu="1"` so text editing keeps native behavior | Centralized |
