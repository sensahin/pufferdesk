...
- `assets/js/core/drag-drop/move-validator.js`: move rules for app/folder/document items, valid containers, duplicate prevention, locked/system items, Trash handling, sidebar bookmarks, and folder nesting.
...
| Desktop | Folder sidebar Favorites | folder, document | `desktop-icons.js`, `app-launcher.js` | Yes | Yes | No | `workspaceState.folderSidebar.items` | Creates bookmarks, not content moves |
...
| Folder pane | Folder sidebar Favorites | folder, document | `app-launcher.js` | Yes | Yes | No | `workspaceState.folderSidebar.items` | Migrated to `DragDropManager` + `MoveService`; creates bookmarks, not content moves |
...
Supported drag/drop item types are currently `app`, `folder`, and `document`. Document items are supported only for sidebar favorite bookmarks today; they are not movable folder contents through `MoveService`. Clipboard commands still own cut/copy/paste for sticky note documents through `assets/js/core/services/clipboard-service.js`.
...
- Moving locked/system folders except adding valid non-Trash folders to sidebar Favorites.
- Moving documents anywhere except sidebar Favorites, where the move creates a bookmark.
