...
Notifications are centralized through `PufferDesk_Notification_Registry`, `PufferDesk_Notification_Controller`, `assets/js/core/notifications/`, and the Desktop API notification facade. Server-side providers can add capability-aware notifications with the `pufferdesk_notifications` filter; browser modules should call `window.PufferDesk.desktopApi.notifications.notify()` for runtime notifications instead of creating standalone toast UI. Notification settings live in the `notifications` settings domain and control source visibility, severity, badges, toasts, quiet mode, sound preference, and history retention. Provider-backed notifications are retained per user and pruned by the saved retention window. Core CSS owns notification structure, while theme CSS owns the visual variables for notification buttons, centers, items, and toasts.
