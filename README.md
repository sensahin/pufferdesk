...
- `assets/js/core/` owns reusable shell behavior through purpose-specific modules:
  ...
  - `search/` owns shared shell search providers, scoring, result normalization, and command-backed result execution for apps, folders, documents, settings, commands, and recents.
...
Sticky Notes use one private WordPress post type (`pufferdesk_note`) and the shared `PufferDesk_Document_Service` for create, duplicate, list, update, and trash behavior. Sticky-note content is stored in `post_content`, while user-visible note/window layout is stored separately in the workspace state. Document organization uses `PufferDesk_Virtual_Filesystem` canonical paths such as `pdk://site/{site}/home/{user}/documents`; the stored `_pufferdesk_document_parent_path` meta is the source of truth for where a saved sticky note appears in virtual folders. Document Open With handlers are exposed through `runtime.documents.openWith`, where each handler declares its app/handler ID and supported document kinds; folders do not receive Open With actions. Do not store sticky-note content or document folder placement in `workspaceState`, browser storage, widgets, or app-specific user meta. Browser code should use `assets/js/core/services/documents.js` and `assets/js/core/services/virtual-filesystem.js` instead of posting directly to document AJAX actions or hard-coding folder paths.
